...
for i in range(len(h2_to_show)):
    ...
    if len(total_spec) != 0:
        lenth = len(total_spec)
        if len(one_spec) == len(total_spec):
            total_spec += np.array(one_spec)
        elif len(one_spec) > lenth:
            total_spec += np.array(one_spec[0:lenth])
        else:
            total_spec[0:len(one_spec)] += np.array(one_spec)
    else:
